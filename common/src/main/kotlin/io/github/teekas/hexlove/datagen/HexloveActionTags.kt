package io.github.teekas.hexlove.datagen
...
class HexloveActionTags(
    output: PackOutput,
    provider: CompletableFuture<HolderLookup.Provider>,
) : TagsProvider<ActionRegistryEntry>(output, HexRegistries.ACTION, provider) {
    override fun addTags(provider: HolderLookup.Provider) {
        // per-world great spells (Requirements 1.9, 1.10): everything else is available immediately
        for (entry in arrayOf(
            HexloveActions.CUPIDS_CHARM,
            HexloveActions.SUCCUBUS_TITHE,
        )) {
