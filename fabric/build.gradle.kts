...
loom {
    runs {
        named("datagen") {
            property("hexlove.apply-datagen-mixin", "true")
        }
    }
}

hexloveModDependencies {
    ...
    optional("modmenu")
    optional(curseforge = "carry-on", modrinth = "carry-on")
    optional("trinkets")
}
...
dependencies {
    ...
    // The API is compile-only: Trinkets remains optional for released Hexlove jars.
    ...
    // Public Carry On 2.1.2.7 data API; compile-only so it remains an optional runtime dependency.
    modCompileOnly(libs.carryon.fabric) { isTransitive = false }
    modLocalRuntime(libs.carryon.fabric) { isTransitive = false }
}
