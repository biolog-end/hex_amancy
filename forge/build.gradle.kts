...
loom {
    forge {
        ...
        mixinConfig(
            "hexlove-common.mixins.json",
            "hexlove-forge.mixins.json",
        )
    }
    ...
    runs {
        for ((platform, outputProject) in arrayOf(
            ...
            register("${platform}Datagen") {
                ...
                property("hexlove.$platform-datagen", "true")
            }
        }
    }
}
...
dependencies {
    ...
    // Curios is present in development for integration checks, but optional for users.
    modCompileOnly(libs.curios.forge) { isTransitive = false }
    modLocalRuntime(libs.curios.forge) { isTransitive = false }
